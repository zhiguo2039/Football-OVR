...
var Ac = {
    ...
    acGetReport: function (ReportKey, ReportSqls, ReportParams, PDFName, aCallback) {
        try {

            if (document.getElementById("TestFlag").checked) {

                var ReviseTime = $('#ReviseTime').val();
                //var ReviseFlag = $('#ReviseFlag').val();
                var FreeText1 = $('#FreeText1').val();
                var FreeText2 = $('#FreeText2').val();
                if (ReviseTime != '') {
                    if (document.getElementById("ReviseFlag").checked) {
                        var ReviseFlag = "0";
                    } else {
                        var ReviseFlag = "-1";
                    }
                ...
                var aDatas = { "ReportKey": ReportKey, "ReportSqls": ReportSqls, "ReportParams": ReportParams, "ReviseParams": { ReviseTime: ReviseTime, ReviseFlag: ReviseFlag, FreeText1: FreeText1, FreeText2: FreeText2 }, "PDFName": PDFName };
                var ActionParas = {
                    Action: "acExportTestFileByCode",
                    Datas: JSON.stringify(aDatas),
                    callback: ""
                }
                $.ajax({
                    url: Ac.Info.SvcUrl,
                ...
                var aDatas = { "ReportKey": ReportKey, "ReportSqls": ReportSqls, "ReportParams": ReportParams, "ReviseParams": { ReviseTime: ReviseTime, ReviseFlag: ReviseFlag, FreeText1: FreeText1, FreeText2: FreeText2 }, "PDFName": PDFName };
                var ActionParas = {
                    Action: "acExportFileByCode",
                    Datas: JSON.stringify(aDatas),
                    callback: ""
                }
                $.ajax({
                    url: "http://127.0.0.1/RS/svcHandler.ashx",
                    data: ActionParas,
                    type: 'POST',
                    success: function (aCallbackData) {
                        try {
    ...
    acGetODF: function (ODFKey, ODFParams, ODFName, aCallback) {
        try {
            var aDatas = { "ODFKey": ODFKey, "ODFParams": ODFParams, "ODFName": ODFName };
            var ActionParas = {
                Action: "acExportODF",
                Datas: JSON.stringify(aDatas),
                callback: ""
    ...
    acImportLocalODF: function (ODFName, aCallback) {
        try {
            var aDatas = { "ODFName": ODFName };
            var ActionParas = {
                Action: "acImportLocalODF",
                Datas: JSON.stringify(aDatas),
                callback: ""
            }
            $.ajax({
